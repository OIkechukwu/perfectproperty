let transactionsCount = document.getElementById('transactions');
...
const fetchAgent = async ()=>{
    try{
        ...
        let {agent} = data;
        console.log(agent)
        //insert the transactions into the DOM
    if (data.status) {
        agent.forEach(agent =>{
            let {
                image
            ...
            transactionsCount.innerHTML = agent.length;
        } else {
            transactionsCount.innerHTML = 0;
        }
    } catch (err){
        console.log(err);
    }
